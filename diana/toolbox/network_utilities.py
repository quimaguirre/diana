...
def get_node_linker_degrees(g, seeds):
    ...
    seed_to_d = {}
    for seed in seeds:
        seed_to_d[seed] = 1 # We will create a dict of the degree of the seeds. It is 1 by default, and it increments by the number of connections that the seed has
    for current_node in g.nodes():
	value = 0 
	for current_neighbor in g.neighbors(current_node): # Obtain the neighbors of the nodes
	    if current_neighbor!=current_node:
		if current_neighbor in seeds: # If the current node has a seed as neighbor, it means that it is a LINKER!! (hurray) and it is awarded with a value of 1 (which is incremented if it has more neighbours (then, it is da BOSS))
		    value += 1
                    seed_to_d[current_neighbor] += 1 
	node_to_ld[current_node] = value # Here we have a nice dict with the nodes in the indexes, and their corresponding values of linkers (or not)
    return node_to_ld, seed_to_d # And the nice dicts are returned for your profit
