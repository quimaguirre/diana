...
import network_utilities, file_converter, stat_utilities, selection_utilities
import os
...
def run_scoring(scoring_folder, random_networks_folder, executable_path, scoring_type="netscore", parameters={"n_iteration":2, "n_repetition":3, "n_sample":100, "sampling_prefix":"./sampled_graph.", "nd_edge_file":"edge_scores_netshort.sif"}, qname=None, calculate_pvalue=True, xval=None, name=None):
	...
	for scoring in ("netscore", "netzcore", "netshort"):
		...
		parameters={"n_iteration":5, "n_sample":100, "sampling_prefix":random_networks_folder+"sampled_graph."}
...
def get_values_from_edges_file_scored(edges_file_scored):
    """
    Parses an edges profile scored and stores the elements into a list of lists:
    [
     [edge1, score, edge2]
     
    ]
    """
    f = open(edges_file_scored)
    edges_file_scored_list = []
    for line in f: 
        edges_file_scored_list.append(line.split())
    f.close()
    return edges_file_scored_list
