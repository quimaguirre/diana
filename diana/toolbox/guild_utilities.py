...
import diana.toolbox.network_utilities as network_utilities # For DIANA
import diana.toolbox.file_converter as file_converter # For DIANA
import diana.toolbox.stat_utilities as stat_utilities # For DIANA
import diana.toolbox.selection_utilities as selection_utilities # For DIANA
import os
...
def run_scoring(scoring_folder, random_networks_folder, executable_path, scoring_type="netscore", parameters={"n_iteration":2, "n_repetition":3, "n_sample":100, "sampling_prefix":"./sampled_graph.", "nd_edge_file":"edge_scores_netshort.sif"}, qname=None, calculate_pvalue=True, xval=None, name=None):
    ...
    def score(scoring_type, qname, node_file, edge_file, output_file, parameters):
        if scoring_type == "netscore": 
            score_command = executable_path + ' -s s -n "%s" -e "%s" -o "%s" -r %d -i %d &> "%s"' % (node_file, edge_file, output_file, parameters["n_repetition"], parameters["n_iteration"], output_file+'.log')
        elif scoring_type == "netzcore": 
            score_command = executable_path + ' -s z -n "%s" -e "%s" -o "%s" -i %d -x %d -d "%s" &> "%s"' % (node_file, edge_file, output_file, parameters["n_iteration"], parameters["n_sample"], parameters["sampling_prefix"], output_file+'.log')
        elif scoring_type == "netshort": 
            score_command = executable_path + ' -s d -n "%s" -e "%s" -o "%s" &> "%s"' % (node_file, parameters["nd_edge_file"], output_file, output_file+'.log')
        else:
    ...
    if scoring_type == "netcombo":
            ...
            if xval is not None:
                for i in range(1, xval+1):
                    ...
                    if not fileExist(output_score_fold_file):
                        score(scoring, qname, node_file+".%d" % i, edge_file, output_score_fold_file, parameters)
            if calculate_pvalue:
                ...
                if xval is not None:
                    for i in range(1, xval+1):
                        ...
                        if not fileExist(output_background_score_fold_file):
                            score(scoring, qname, bg_node_file+".%d" % i, edge_file, output_background_score_fold_file, parameters)
        ...
        if xval is not None:
            for i in range(1, xval+1):
                ...
                score(scoring_type, qname, node_file+".%d" % i, edge_file, output_score_fold_file, parameters)
        if calculate_pvalue:
            ...
            if xval is not None:
                for i in range(1, xval+1):
                    ...
                    score(scoring_type, qname, bg_node_file+".%d" % i, edge_file, output_background_score_fold_file, parameters)
                    output_pvalue_file(output_score_fold_file, output_background_score_fold_file, None, bg_seed_file)
    return
