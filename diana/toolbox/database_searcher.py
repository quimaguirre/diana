...
def get_smiles_from_dcdbid(dcdbid, toolbox_dir):
    """
    Searches the SMILES of a DCDB drug using BIANA
    """

    dump_file = os.path.join(toolbox_dir, "dcdb2smiles.pcl")
...
def search_seeds_in_file(dcdbid, toolbox_dir, provided_seeds = None):
    """
    ...
    dump_file = os.path.join(toolbox_dir, "dcdb2targets.pcl")
...
def search_pfams_for_seeds(seeds, database, toolbox_dir):
    """
    Performs a search in the database to find the PFAMs of the seeds.
    Returns a set of PFAMs
    """

    dump_file = os.path.join(toolbox_dir, "geneid2pfam.pcl")
