...
def main():
    ...
    print("  DIANA INFO:\tTop threshold= %d%%\n"%(top_threshold))
    print("  DIANA INFO:\tP-value threshold= %f\n"%(pval_threshold))
    ...
    #print("  DIANA INFO:\tTop percentage threshold= %d%% top scoring nodes are first %d nodes\n"%(top_threshold,ntop))  
    ii=0

    # If entry_type == "uniprotentry" or "geneid", start writing a node profile
    if (entry_type == "uniprotentry" or entry_type == "geneid"):
        f = open(node_profile_file, 'w')
        delim = " "
        f.write("#Id%sScore%sP-value\n" % (delim, delim))
    
    # Now, filter top_nodes by p-value <= (0.05). Include them in the profile if entry_type == "uniprotentry" or "geneid"
    ...
    print ("  DIANA INFO:\t%d proteins in network, %d in top %d%% scoring proteins, and %d under the p-value threshold of %f\n" %
...
#     os.system(command)
    ...
    if (entry_type == "refseq" or entry_type == "geneid"):
        f = open(enrichment_file, 'w')
        functional_enrichment.check_functional_enrichment(list(top_nodes_filtered), list(all_nodes), entry_type,
