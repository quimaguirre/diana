...
def main():
    ...
    subnetwork_file = scoring_folder + "edges_profile.sif"
    ...
    print("Top percentage threshold= %d are first %d nodes\n"%(top_threshold,ntop))
    ii=0
    last_score = ''
    ...
    node_ld = network_utilities.get_node_linker_degrees(g, seeds) # Finds if the nodes in the network are linkers or not and give them a value (+1 for the each seed that is neighbor of the node)
    ...
    for node,ld in node_ld.iteritems(): # This loop adds the linkers found previously (degree > 0) into the ld_nodes set where the seeds were introduced before
        if ld>0:
            ld_nodes.add(node)
    g_link = network_utilities.get_subgraph(g, ld_nodes) # Now, a subnetwork of the subnetwork is created using the linkers, obtaining a NETWORK of the LINKERS!!
