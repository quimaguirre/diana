...
def parse_user_arguments(*args, **kwds):
    ...
    parser.add_argument('-tra','--trans_if_sif',dest='trans_if_sif',action = 'store',
                        help = 'If sif file introduced, introduce the translation file if possible (optional)')
    ...
    parser.add_argument('-ex','--extended',dest='extended',action = 'store', default=None,
                        ...
                        Default: None
                        """)
    parser.add_argument('-db','--database',dest='database',action = 'store',default='BIANA_MARCH_2013',
                        help = """Define the database to use for the generation of the network of expansion. There are two options: 
                        - BIANA_MARCH_2013
                        - BIANA_2016
                        (default is BIANA_MARCH_2013)""")
...
def run_generate_netscore_files(drug, seeds_file, radius, taxid, restriction, seeds_type, translation_type, database):
    ...
    if seeds_type == "geneid":
        command = "/soft/devel/python-2.7/bin/python2.7 toolbox/generate_netscore_files_geneID%s.py -iseed %s -radius %d -stype %s -ttype %s -score 0.0 -taxid %d -node %s -loc %s -edge %s -trans %s -format guild %s " % (db, seeds_file, radius, seeds_type, translation_type, taxid, nodes, loc, edges, trans, restriction)
    elif seeds_type == "uniprotentry":
        command = "/soft/devel/python-2.7/bin/python2.7 toolbox/generate_netscore_files%s.py -iseed %s -radius %d -stype %s -ttype %s -score 0.0 -taxid %d -node %s -loc %s -edge %s -trans %s -format guild %s " % (db, seeds_file, radius, seeds_type, translation_type, taxid, nodes, loc, edges, trans, restriction)
    os.system(command)
...
def run_translate_network(network_input_file, nodes_input_file, taxid, translation_type, output_file, database, translation_of_nodes_file = None):
    ...
    if (translation_of_nodes_file == None):
        ...
        command = "/soft/devel/python-2.7/bin/python toolbox/TranslateNetwork%s.py -i %s -n %s -trans %s -x -taxid %d -ttype %s -o %s" % (db, network_input_file, nodes_input_file, translation_of_nodes_file, taxid, translation_type, output_file)
    os.system(command)
...
def run_diana(options):
    ...
    for (drug, seeds) in list_seeds_drugs:
        ...
        # If the seeds file is provided, the seeds are processed
            ...
            #     (seeds_geneid, seeds_file_geneid) = translate_seeds(seeds, options.taxid, drug)
            ...
            if len(seeds) < 3:
                ...
                name = detect_special_drug_name(drug)
                # If the type of seeds provided is geneid, they are translated to uniprotentry, because currently it is only possible to extract uniprotentry seeds from DrugBank
                if seeds_type == "geneid":
                    (seeds, seeds_file) = translate_seeds(seeds, "geneid", "uniprotentry", options.taxid, drug, options.database)
                    seeds_type = check_type_of_seed(seeds)
    ...
    for drug in (options.drug1, options.drug2):
        ...
        if options.sif == None:
            ...
            if options.type_if_sif != 'geneid' or options.trans_if_sif == None:
                ## Translation of the SIF file to GENEID in order to run the functional analysis
    ...
    # If speed is 1, the comparison of the Edge profile will be performed
    if options.speed == 1:
    ...
    if skip_ddi == False:
        ...
        if options.drug1 != 'drug1' and options.drug2 != 'drug2':
            ...
            name1 = detect_special_drug_name(options.drug1)
            name2 = detect_special_drug_name(options.drug2)
            ...
            if description1 == [] and description2 ==[]:
                ...
                if description1 == description2:
                    ...
                    for description in description1:
                        print("\t\t%s" %(description))
                    print("")
                else:
    ...
    if options.speed == 1:
        output_file_general = results_folder+'/general_extended_analysis_%s_%s.txt' %(options.drug1, options.drug2)
        output_file_linker = results_folder+'/linker_extended_analysis_%s_%s.txt' %(options.drug1, options.drug2)
    elif options.speed == 2:
        output_file_general = results_folder+'/general_extended_analysis_SPEED%d_%s_%s.txt' %(options.speed, options.drug1, options.drug2)
    if options.extended != None:
    ...
    fr.write("\n#### RESULTS ####\n")
    fr.write("# Spearman\tP-value\tDot Product\n")
    ...
    fr.write("%0.3f\t%0.3f\t%0.3f\n" %(summary_seed_node[1],summary_seed_node[2], summary_seed_node[0]))
