...
def run_generate_netscore_files(drug, seeds_file, radius, taxid, restriction, seeds_type, translation_type, database):
    ...
    command = "/aplic/GOOLF/1.6.10/Python/2.7.9/bin/python toolbox/generate_netscore_files%s%s.py -iseed %s -radius %d -stype %s -ttype %s -score 0.0 -taxid %d -node %s -loc %s -edge %s -trans %s -format guild %s " % (addgeneid, db, seeds_file, radius, seeds_type, translation_type, taxid, nodes, loc, edges, trans, restriction)
    os.system(command)
...
def run_score_network(network_file, pvalue_file, seed_file, top_threshold, pval_threshold, data_dir):
    ...
    command = "/aplic/GOOLF/1.6.10/Python/2.7.9/bin/python toolbox/score_network.py {} {} {} {:f} {:f} {}".format( network_file, pvalue_file, seed_file, top_threshold, pval_threshold, data_dir )
    os.system(command)
...
def run_top_scoring(data_dir, pvalue_file, top_threshold, pval_threshold, analysis_type):
    ...
    command = "/aplic/GOOLF/1.6.10/Python/2.7.9/bin/python  toolbox/top_scoring_combination.py %s %s %s %f %f %s " % (data_dir, pvalue_file, data_dir, top_threshold, pval_threshold, analysis_type)
    os.system(command)
...
def run_transform_network(network_input_file, nodes_input_file, translation_file, output_file):
    ...
    command = "/aplic/GOOLF/1.6.10/Python/2.7.9/bin/python toolbox/TransformNetwork.py -i %s -n %s -trans %s -oe %s -on %s" % (network_input_file, nodes_input_file, translation_file, output_edges_file, output_nodes_file)
    os.system(command)
...
def run_translate_network(network_input_file, nodes_input_file, taxid, translation_type, output_file, database):
    ...
    command = "/aplic/GOOLF/1.6.10/Python/2.7.9/bin/python toolbox/TranslateNetwork%s.py -i %s -n %s -x -taxid %d -ttype %s -o %s" % (db, network_input_file, nodes_input_file, taxid, translation_type, output_file)
    os.system(command)
...
def translate_seeds(seeds_list, input_type, translation_type, taxid, drug, database):
    ...
    if input_type == "uniprotentry":
        command = "/aplic/GOOLF/1.6.10/Python/2.7.9/bin/python toolbox/TranslateNetwork%s.py -i %s -n %s -iformat %s -x -taxid %d -ttype %s -o %s" % (db, seeds_file, seeds_file, input_type, taxid, translation_type, output_file)
    elif input_type == "geneid":
        command = "/aplic/GOOLF/1.6.10/Python/2.7.9/bin/python toolbox/TranslateNetwork%s.py -i %s -n %s -iformat %s -xf -taxid %d -ttype %s -o %s" % (db, seeds_file, seeds_file, input_type, taxid, translation_type, output_file)
    os.system(command)
...
def run_diana(options):
	...
	#--------------------------------------------#
        ...
        for drug in (options.drug1, options.drug2):
            ...
            if not fileExist(nodes_file) and not fileExist(edges_file):
                ...
                guild_command = "/aplic/GOOLF/1.6.10/Python/2.7.9/bin/python toolbox/run_guild.py %s %s %s %s %s" % (data_dir, new_seeds_file, sif_file, results_dir, random_networks_dir)
	...
	#-------------------------------#
        ...
        for drug in (options.drug1, options.drug2):
            ...
            if not fileExist(pval_file):
                ...
                guild_command = "/aplic/GOOLF/1.6.10/Python/2.7.9/bin/python toolbox/run_guild.py %s %s %s %s %s" % (data_dir, new_seeds_file, options.sif, results_dir, random_networks_dir)
                os.system(guild_command)
    ...
    if comparison_mode == True or normal_mode == True:
        ...
        # Get top 50 functions from profile and compare them
        top = 50
        top_func_dict_drug1 = comp.get_top_functions(enrichment_drug1, top)
        top_func_dict_drug2 = comp.get_top_functions(enrichment_drug2, top)
        summary_seed_functional_top50 = comp.calculate_comparison(top_func_dict_drug1, top_func_dict_drug2)

        # Get top 100 functions from profile and compare them
        top = 100
        top_func_dict_drug1 = comp.get_top_functions(enrichment_drug1, top)
        top_func_dict_drug2 = comp.get_top_functions(enrichment_drug2, top)
        summary_seed_functional_top100 = comp.calculate_comparison(top_func_dict_drug1, top_func_dict_drug2)
    ...
    top_func_dict_drug1 = comp.get_top_functions(enrichment_drug1, top)
    top_func_dict_drug2 = comp.get_top_functions(enrichment_drug2, top)
    summary_top_functional = comp.calculate_comparison(top_func_dict_drug1, top_func_dict_drug2)
    summary["top50_functional"] = summary_top_functional
    ...
    top_func_dict_drug2 = comp.get_top_functions(enrichment_drug2, top)
    summary_top_functional = comp.calculate_comparison(top_func_dict_drug1, top_func_dict_drug2)
    summary["top100_functional"] = summary_top_functional
    ...
    # Get top 50 functions from profile and compare them
    top = 50
    top_func_dict_drug1 = comp.get_top_functions(linker_functional_profile_drug1, top)
    top_func_dict_drug2 = comp.get_top_functions(linker_functional_profile_drug2, top)
    summary_top_functional = comp.calculate_comparison(top_func_dict_drug1, top_func_dict_drug2)
    summary["top50_linker_functional"] = summary_top_functional
    ...
    top_func_dict_drug2 = comp.get_top_functions(linker_functional_profile_drug2, top)
    summary_top_functional = comp.calculate_comparison(top_func_dict_drug1, top_func_dict_drug2)
    summary["top100_linker_functional"] = summary_top_functional
    ...
    for top in extended_dataframe['functional']:
        fot.write('{}\t{}\t'.format(top[1], top[3]))
        fot.write('{}\t{}\t{}\t{}\t'.format(top[7], top[8], top[9], top[10])) # Top 50 Sp and Dp and Top 100 Sp and Dp
    ...
    fot.write('{}\t{}\t{}\t{}\t'.format(summary['top50_linker_functional'][1], summary['top50_linker_functional'][0], summary['top100_linker_functional'][1], summary['top100_linker_functional'][0])) # Top 50 Sp and Dp and Top 100 Sp and Dp
    ...
    fot.write('{}\t{}\t'.format(summary_seed_functional_top50[1], summary_seed_functional_top50[0])) # Top 50 Sp and Dp
    fot.write('{}\t{}\t'.format(summary_seed_functional_top100[1], summary_seed_functional_top100[0])) # Top 100 Sp and Dp
