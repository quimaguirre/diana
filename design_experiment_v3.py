...
bash_file = "diana_drug_comb_100_analyses_v2.sh"
...
for ddi_pair in ddi_pairs:
    ...
    command = "/soft/devel/python-2.7/bin/python diana_v3.2.py -d1 %s -d2 %s -sif %s -tis geneid -ex top_thresholds.list -db %s 2>&1 | tee logs/%s_%s.log\n" %(drug1, drug2, sif_file, database, drug1, drug2)
    f.write(command)
