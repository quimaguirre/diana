...
crossings_file = "diana_crossings_cluster_more5targets.txt"
...
while (n < len(list_of_drugs)):
    ...
    while (i < len(list_of_drugs2)):
        ...
        if ddi_name1 not in crossings and ddi_name2 not in crossings:
            ...
            pair2comb[ddi_name1] = 0 # We will introduce 0 if it is not drug combination
            non_dc+=1
            for drug_comb in drug_comb_2_drugs:
                if drug1 in drug_comb_2_drugs[drug_comb] and drug2 in drug_comb_2_drugs[drug_comb]:
                    pair2comb[ddi_name1] = 1 # We will introduce 1 if it is a drug combination
                    dc+=1
                    non_dc-=1
                    break
...
print('NUMBER OF DRUG COMBINATIONS:\t\t{}'.format(dc))
print('NUMBER OF NON-DRUG COMBINATIONS:\t{}'.format(non_dc))
