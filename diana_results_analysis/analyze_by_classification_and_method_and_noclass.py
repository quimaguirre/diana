...
def parse_results():
    ...
    plot_name = results_dir + '/classification_analysis_noclass.eps'
    ...
    types_analysis = ['all', 'guild', 'linkers', 'seeds', 'struc']
    ...
    #figsize = (len(classifications)*2, 8)
    fig = figure(dpi=300)
    ax = axes()
    #hold(True)
    pos = 1
    ...
    for classification in classifications:
        ...
        for x in xrange(len(types_analysis)):
            for x in xrange(len(['in classification', 'not in classification'])):
        ...
        data = []
        for type_analysis in types_analysis:
            for analysis in ['in classification', 'not in classification']:
                data.append(results_analysis[classification][type_analysis][analysis]['all_aucs']) # Get the groups of plots that we will add

        # Boxplot group
        #bp = boxplot(data, positions = positions, widths = 0.6)
        bp = boxplot(data, positions = positions, widths = 0.6, patch_artist=True)
        setBoxColors(bp, len(types_analysis)*len(['in classification', 'not in classification']))
    ...
    legend([hB, hG, hY, hR, hBl],types_analysis)
...
def setBoxColors(bp, length):
    ...
    """
    if length != 10:
        print('\nERROR: This function is thought to work on groups of 10 types of analysis, not {}. Redo the function!!\n'.format(length))
        sys.exit(10)
    ...
    bp['boxes'][1].set_facecolor('#9ed0ff')
    ...
    setp(bp['boxes'][6], color='#b80000')
    setp(bp['caps'][12], color='#b80000')
    setp(bp['caps'][13], color='#b80000')
    setp(bp['whiskers'][12], color='#b80000')
    setp(bp['whiskers'][13], color='#b80000')
    setp(bp['medians'][6], color='black')
    bp['boxes'][6].set_facecolor('#ff7373') #red
    ...
    setp(bp['boxes'][8], color='black')
    setp(bp['caps'][16], color='black')
    setp(bp['caps'][17], color='black')
    setp(bp['whiskers'][16], color='black')
    setp(bp['whiskers'][17], color='black')
    setp(bp['medians'][8], color='#fbc562')
    bp['boxes'][8].set_facecolor('grey')

    setp(bp['boxes'][9], color='black')
    setp(bp['caps'][18], color='black')
    setp(bp['caps'][19], color='black')
    setp(bp['whiskers'][18], color='black')
    setp(bp['whiskers'][19], color='black')
    setp(bp['medians'][9], color='#fbc562')
    bp['boxes'][9].set_facecolor('grey')
