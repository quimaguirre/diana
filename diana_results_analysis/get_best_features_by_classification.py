...
def parse_results():
    ...
    auc_results_table_tsv = results_dir + '/best_features_diffrelproc.tsv'
    #auc_results_table_tsv_groups = results_dir + '/best_features_diffrelproc.tsv'
...
#        for an in ('sp', 'dp', 'ji'):
    ...
    df = df.ix[class_rows] # Create a table with the rows selected (DDIs of the class and non-DDIs)
    ...
    # for (name, group) in [ ('GUILD node',guild_node), ('GUILD edge',guild_edge), ('GUILD functions',guild_func), ('Linkers', link_info), ('Seeds', seed_info) ]:
    ...
    #             fo2.write('{}\t{}\n'.format( feature, results['AUC crossval']))
