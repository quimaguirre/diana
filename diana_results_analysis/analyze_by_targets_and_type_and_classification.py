...
def parse_results():
    ...
    plot_name = results_dir + '/figures/targets_and_type_{}_diffrelpathways.eps'.format(greater_or_smaller)
    ...
    node_info = ['Nit500sp', 'Nit1000dp', 'LNdp', 'LNsp', 'SNdp', 'SNji']
    edge_info = ['Eit1000sp', 'Eit50dp', 'LEsp', 'LEdp']
    func_info = ['Fit500dp', 'Fper0.1sp', 'LFdp', 'LFsp', 'SFdp', 'SFsp']
    ...
    for num in num_targets:
        ...
        for (df_analysis, analysis) in [(df_class, 'in classification')]:
            ...
            for type_analysis in types_analysis:
                ...
                df_all = df_analysis[node_info+edge_info+func_info+['Struc']+['Comb']]
                df_node = df_analysis[node_info+['Comb']]
                df_edge = df_analysis[edge_info+['Comb']]
                df_func = df_analysis[func_info+['Comb']]
    ...
    legend([hG, hR, hB, hBl, hW],types_analysis)
...
def setBoxColors(bp, length):
    ...
    bp['boxes'][3].set_facecolor('grey') #grey
    ...
    setp(bp['medians'][4], color='grey')
    bp['boxes'][4].set_facecolor('#e5e5e5') #light grey
