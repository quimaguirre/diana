...
def parse_results():
    ...
    plot_name = results_dir + '/figures/classification_and_method{}_presentation.eps'.format(me_too_str)
    plot_name_png = results_dir + '/figures/classification_and_method{}_presentation.png'.format(me_too_str)

    # Ranking of mean of scores for drug combinations list
    drug_comb_scores_file = results_dir + '/tables/drug_comb_scores_list_classification_and_method{}.txt'.format(me_too_str)
    ...
    acc_sens_all = results_dir + '/figures/classification_and_method{}_accsens_all.eps'.format(me_too_str)
    acc_sens_guild = results_dir + '/figures/classification_and_method{}_accsens_guild.eps'.format(me_too_str)
    acc_sens_linkers = results_dir + '/figures/classification_and_method{}_accsens_linkers.eps'.format(me_too_str)
    acc_sens_seeds = results_dir + '/figures/classification_and_method{}_accsens_seeds.eps'.format(me_too_str)
    acc_sens_struc = results_dir + '/figures/classification_and_method{}_accsens_struc.eps'.format(me_too_str)
    acc_sens_random = results_dir + '/figures/classification_and_method{}_accsens_random.eps'.format(me_too_str)
    ...
    mannwhitney_file = results_dir + '/tables/classification_and_method{}_mannwhitney.txt'.format(me_too_str)
    mannwhitney_struc_file = results_dir + '/tables/classification_and_method{}_mannwhitneystruc.txt'.format(me_too_str)

    # Results table:
    results_table = results_dir + '/tables/classification_and_method{}_table.txt'.format(me_too_str)

    # Results table:
    prec_rec_table = results_dir + '/tables/classification_and_method{}_precrec_table.txt'.format(me_too_str)
    ...
    types_analysis = ['guild', 'seeds', 'struc', 'random']
    types_analysis_labels = ['Guild', 'Targets', 'Structure', 'Random']
    ...
    for classification in classifications:
        ...
        scores_fd.write('##### {} #####\n'.format(classification))
        scores_best_predictions = results_analysis[classification][method_best_predictions]['in classification']['all_probs']
    ...
    lgd = ax.legend(handles=(hG, hR, hBl, hW), labels=types_analysis_labels, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
    ...
    savefig(plot_name_png, format='png', bbox_extra_artists=(lgd,), bbox_inches='tight')
    show()
    ...
    # Header
    fp.write(' ')
    types_analysis2 = ['guild', 'seeds', 'struc'] # Without random!
...
def setBoxColors(bp, length):
    ...
    setp(bp['medians'][0], color='black')
    bp['boxes'][0].set_facecolor('#32f232')
    ...
    setp(bp['medians'][2], color='#fbc562')
    bp['boxes'][2].set_facecolor('#4f4f4f')
    ...
    bp['boxes'][3].set_facecolor('#aeaeae')
