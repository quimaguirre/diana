...
def main():
    ...
    seed_file = data_dir + "seeds_uniprotentry.txt"
    seed2_file = data_dir + "secondary.txt"
    network_file = data_dir + "interactions_uniprotentry.sif"
    ...
    subnetwork_linker2_file = scoring_folder + "subnetwork_linkers_secondary.sif"
    linker2_file = scoring_folder + "linkers_secondary.txt"
	...
	score, pval = vals
	    ...
	    top_nodes.add(node)
            ...
            last_value=score
            ii=ii+1
        if ii >= ntop and score==last_value:
	    top_nodes.add(node)
            print("Add Node %s Score %e P-value: %e\n"%(node,score,pval))
            ii=ii+1
    ...
    # Get subnetwork linkers induced by a second set of relevant nodes (second seeds) and add the primary seed-linkers
    seeds2=set()
    fs = open(seed2_file,"r")
    for line in fs:
      seeds2.add(line.strip().split()[0])
    fs.close()
    ld2_nodes=seeds2.intersection(top_nodes)
    ld2_nodes.update(ld_nodes)
    node_ld2 = network_utilities.get_node_linker_degrees(g_sub, seeds2)
    sorted_nodes_by_ld2 = sorted(node_ld2.items(), key=lambda x: x[1],reverse=True)
    for node,ld in node_ld2.iteritems():
      if ld>0:
       ld2_nodes.add(node)
    g_link2 = network_utilities.get_subgraph(g_sub, ld2_nodes)
    ...
    fl2 = open(subnetwork_linker2_file, 'w')
    ...
    # output the linkers of the second set
    fn = open(linker2_file, 'w')
    for node,ld in sorted_nodes_by_ld2:
      fn.write("%s\t%d\t%f\t%e\n" % (node,ld,node_to_vals[node][0],node_to_vals[node][1]))
    fn.close()
