...
def extract(options):
    ...
    print "Open session"
    session = create_new_session(sessionID="biana_session",dbname="BIANA_MARCH_2013",dbhost="sbi.upf.edu",
        ...
        if not fileExist(options.seed):
            ...
            if options.taxid != "0":
             ...
             proteome = session.create_new_user_entity_set(  identifier_description_list =seed_list,
                                                          id_type=options.stype,new_user_entity_set_id="proteome",
                                                          negative_attribute_restriction_list=[] )
    # If radius == 0, whole network created
    else:
        level=0
    ...
    if not fileExist(options.seed):
        ...
        for protein in nodes:
            ...
            if options.ttype == "proteinsequence":
                ...
                #translation=",".join([str(current_id) for current_id in uE.get_attribute(attribute_identifier=options.ttype)]) # old procedure without seeds
