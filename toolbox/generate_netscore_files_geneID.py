...
def parse_user_arguments(*args, **kwds):
    ...
    parser.add_argument('-ttype','--translation_type',dest='ttype',action = 'store',default='genesymbol',
                        help = '''Type of identifier for the output translation of codes (default is genesymbol)
...
def extract(options):
    ...
    session = create_new_session(sessionID="biana_session",dbname="BIANA_MARCH_2013",dbhost="ben-yehuda.upf.edu",
...
#Summary of interactions
    ...
    if not fileExist(options.input_array_annotation):
         ...
         for line in fd:
             fields = line.strip().split("\t")
             geneid2microarray.setdefault(fields[2], set()).add(fields[0])
             genesymbol2microarray.setdefault(fields[1].lower(), set()).add(fields[0])
             #print ("Check GID %s SYMB: %s CODE %s\n"%(fields[2],fields[1],fields[0]))
         fd.close()
         for protein in nodes:
             for microarray_codes in translations.setdefault(protein, get_translation(protein,session,geneid2microarray,genesymbol2microarray)):
                 out_annotation.write("{0}\t{1}\n".format(protein, microarray_codes))
    out_annotation.close()

    # If we do not provide seeds (we want the whole network)
    if not fileExist(options.seed):
...
def get_translation(uE_id,session,geneid2microarray,genesymbol2microarray):
 uE = session.get_user_entity(uE_id)
 microarray_codes = set()
 for current_gid in uE.get_attribute(attribute_identifier="geneid"):
  if current_gid.value in geneid2microarray:
    microarray_codes.update(geneid2microarray[current_gid.value])
