...
def analysis_results(options):
    ...
    img_dir = os.path.join(options.workspace, 'figures')
    create_directory(img_dir)
    fig_format = 'png'
    plot_name = os.path.join(img_dir, 'evaluation_classifiers_with_clinicaltrials.{}'.format(fig_format))
    classifiers = {
    ...
    if not fileExist(plot_name):
        ...
        ndc_training_groups = obtain_n_groups_of_k_length(ndc_data, repetitions, num_dc) # Obtain n number of groups containing different non-drug combinations to repeat the analysis n times

        ndc_testing_groups = obtain_n_groups_of_k_length(ndc_ct_data, repetitions, num_ct_dc)

        for classifier in classifiers:

            print('Classifier: {}\n'.format(classifier))

            mean_aucs, var, std_aucs, all_aucs = run_machine_learning_scikit(classifiers[classifier], repetitions, dc_data, ndc_training_groups, dc_ct_data, ndc_testing_groups)
