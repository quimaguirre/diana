...
def analysis_results(options):
    ...
    plot_auc_distribution = os.path.join(img_dir, 'numtargets_auc_distribution{}{}.{}'.format(atc_str, pca_str, fig_format))

    # Plot of accuracy/sensitivity name
    acc_sens_dctargets = os.path.join(img_dir, 'numtargets_accsens_dctargets{}{}.{}'.format(atc_str, pca_str, fig_format))
    ...
    acc_sens_dcatc = os.path.join(img_dir, 'numtargets_accsens_dcatc{}{}.{}'.format(atc_str, pca_str, fig_format))
    acc_sens_dcse = os.path.join(img_dir, 'numtargets_accsens_dcse{}{}.{}'.format(atc_str, pca_str, fig_format))

    # Results table
    results_table = os.path.join(tables_dir, 'numtargets_auc_table{}{}.txt'.format(atc_str, pca_str))

    # Accuracy/Sensitivity results table
    prec_rec_table = os.path.join(tables_dir, 'numtargets_accsens_table{}{}.txt'.format(atc_str, pca_str))

    # File with results of Mann Whitney tests
    mannwhitney_file = os.path.join(tables_dir, 'numtargets_mannwhitney{}{}.txt'.format(atc_str, pca_str))
    ...
    for num in num_targets:
        ...
        for index, row in df.iterrows():
            ...
            if greater_or_smaller == 'greater':
                if len(drugbank_to_targets[drug1]) >= num and len(drugbank_to_targets[drug2]) >= num:
                    selected_rows.append(index)
    ...
    plot_auc_distributions(analysis_results, num_targets, types_analysis, types_analysis_labels, plot_auc_distribution, fig_format=fig_format, consider_se=consider_se, different_atc=options.different_atc)
...
def plot_violin(analysis_results, num_targets, types_analysis, types_analysis_labels, plot_name, greater_or_smaller='greater', fig_format='png', consider_se=False):
    ...
    if consider_se:
        colors_ordered = [['#ff7373', 'red'], ['#32f232', 'green'], ['#4f4f4f', 'black'], ['#22a9bd', '#0049e5'], ['#e59600', '#966200'], ['#aeaeae', 'black']] # yellow, red, green, black, blue, orange, grey
    else:
        colors_ordered = [['#ff7373', 'red'], ['#32f232', 'green'], ['#4f4f4f', 'black'], ['#aeaeae', 'black']] # white, red, green, black, grey
    ...
    for num in num_targets:
        ...
        for x in xrange(len(types_analysis)):
            ...
            # Get data
            data=analysis_results[num][types_analysis[x]]['all_aucs']
            ...
            for pc in parts['bodies']:
                ...
                pc.set_edgecolor(colors_ordered[x][1])
    ...
    # Save
    pylab.savefig(plot_name, format=fig_format, bbox_inches='tight')
    pylab.show()


def plot_auc_distributions(analysis_results, num_targets, types_analysis, types_analysis_labels, plot_name, greater_or_smaller='greater', fig_format='png', consider_se=False, different_atc=False):
    ...
    for num in num_targets:
        ...
        data = []
        for method in types_analysis:
            data.append(analysis_results[num][method]['all_aucs']) # Get the groups of plots that we will add
