...
import diana.indigo_package.indigo as indigo
import diana.toolbox.comparison as comp
import diana.toolbox.database_searcher as dbs
...
def parse_user_arguments(*args, **kwds):
    ...
    parser.add_argument('-ex','--extended',dest='extended',action = 'store', default=None,
                        ...
                        - A file with different threshold values separated by commas, and the type of threshold separated by a tab. 
                        - There are two types of thresholds: 'percentage' and 'item'
                        For example, a file called "top_thresholds.list" containing:
                        0.1,0.5,1,2.5,5,10,20\tpercentage
                        50,100,250,500,1000\titem
                        Default: None
                        """)
    ...
    parser.add_argument('-ws','--worspace',dest='workspace',action = 'store',default=os.path.join(os.path.dirname(__file__), 'workspace'),
                        help = """Define the workspace directory where the data directory and the results directory will be created""")
    parser.add_argument('-prof','--profile',dest='profile_creation',action = 'store_true',
                        help = """ Profile creation: It only creates the profile of the first drug specified. It does not run the comparison.
                        If profile creation and comparison are specified together, it will both create the profiles for both drugs and compare them.
                        If any of them is specified, it will happen the same
                        """)
    parser.add_argument('-comp','--comparison',dest='comparison',action = 'store_true',
                        help = """ Comparison: It only compares the profiles of the two drugs. It does not create any profile.
                        If profile creation and comparison are specified together, it will both create the profiles for both drugs and compare them.
                        If any of them is specified, it will happen the same
                        """)
...
def find_seeds_in_nodes_file(drug, nodes_file, new_seeds_file):
    ...
    command = "cp "+new_seeds_file+" "+os.path.join(options.workspace, "data", drug, "guild_results/seeds.txt")
    os.system(command)
...
def run_generate_netscore_files(data_dir, drug, seeds_file, radius, taxid, restriction, seeds_type, translation_type, database):
    """Runs the program generate_netscore_files.py that generates a network of expansion from a seeds file"""

    nodes = os.path.join(data_dir, drug+"/human_nodes_BIANA_"+drug+".txt")
    loc = os.path.join(data_dir, drug+"/noLoc_bPPI_"+drug+".txt")
    edges = os.path.join(data_dir, drug+"/human_edges_BIANA_"+drug+".txt")
    trans = os.path.join(data_dir, drug+"/translation_BIANA_"+drug+".txt")
    radius = int(radius)
    taxid = int(taxid)
    if (restriction == 'eAFF'):
        restriction = '-eAFF'
    ...
    command = "python %s/diana/toolbox/generate_netscore_files%s%s.py -iseed %s -radius %d -stype %s -ttype %s -score 0.0 -taxid %d -node %s -loc %s -edge %s -trans %s -format guild %s " % (script_path, addgeneid, db, seeds_file, radius, seeds_type, translation_type, taxid, nodes, loc, edges, trans, restriction)
    os.system(command)
...
def run_score_network(network_file, pvalue_file, seed_file, top_threshold, pval_threshold, data_dir):
    ...
    command = "python {}/diana/toolbox/score_network.py {} {} {} {:f} {:f} {}".format( script_path, network_file, pvalue_file, seed_file, top_threshold, pval_threshold, data_dir )
    os.system(command)
...
def run_top_scoring(data_dir, pvalue_file, top_threshold, pval_threshold, analysis_type):
    ...
    command = "python  %s/diana/toolbox/top_scoring_combination.py %s %s %s %f %f %s " % (script_path, data_dir, pvalue_file, data_dir, top_threshold, pval_threshold, analysis_type)
    os.system(command)
...
def run_transform_network(network_input_file, nodes_input_file, translation_file, output_file):
    ...
    command = "python %s/diana/toolbox/TransformNetwork.py -i %s -n %s -trans %s -oe %s -on %s" % (script_path, network_input_file, nodes_input_file, translation_file, output_edges_file, output_nodes_file)
    os.system(command)
...
def run_translate_network(network_input_file, nodes_input_file, taxid, translation_type, output_file, database):
    ...
    command = "python %s/diana/toolbox/TranslateNetwork%s.py -i %s -n %s -x -taxid %d -ttype %s -o %s" % (script_path, db, network_input_file, nodes_input_file, taxid, translation_type, output_file)
    os.system(command)
...
def translate_seeds(seeds_list, input_type, translation_type, taxid, drug, database):
    ...
    try:
        os.stat(os.path.join(options.workspace, "seeds"))
    except:
        print("  DIANA INFO:\tCreating directory for seeds.\n")
        os.mkdir(os.path.join(options.workspace, "seeds"))
    ...
    if input_type == "uniprotentry":
        command = "python %s/diana/toolbox/TranslateNetwork%s.py -i %s -n %s -iformat %s -x -taxid %d -ttype %s -o %s" % (script_path, db, seeds_file, seeds_file, input_type, taxid, translation_type, output_file)
    elif input_type == "geneid":
        command = "python %s/diana/toolbox/TranslateNetwork%s.py -i %s -n %s -iformat %s -xf -taxid %d -ttype %s -o %s" % (script_path, db, seeds_file, seeds_file, input_type, taxid, translation_type, output_file)
    os.system(command)
...
def run_diana(options):
    ...
    # Get the script path
    script_path = os.path.dirname(__file__)
    toolbox_dir = os.path.join(script_path, 'diana/toolbox')

    # Inform if PROFILE CREATION or COMPARISON are specified
    if options.profile_creation == True and options.comparison == False:
        profile_creation_mode = True
        comparison_mode = False
        normal_mode = False
        print("  DIANA INFO:\tPROFILE CREATION MODE. It will only create the profile of the first drug\n")
    elif options.profile_creation == False and options.comparison == True:
        comparison_mode = True
        profile_creation_mode = False
        normal_mode = False
        print("  DIANA INFO:\tCOMPARISON MODE. It will only compare the profiles of the two drugs\n")
    elif (options.profile_creation == False and options.comparison == False) or (options.profile_creation == True and options.comparison == True):
        normal_mode = True
        profile_creation_mode = False
        comparison_mode = False
        print("  DIANA INFO:\tNORMAL MODE. It will create the profiles of the two drugs and compare them\n")

    # Inform if EXTENDED analysis selected
    if options.extended != None:
        print("  DIANA INFO:\tEXTENDED ANALYSIS ACTIVATED\n")

    # Load the files go and gene2go files for the functional analysis
    if profile_creation_mode == True or normal_mode == True:
        print("  DIANA INFO:\tLoading GOATOOLS for the functional analysis. Remember to update the files go-basic.obo and gene2go.\n")
        print("\t\twget http://geneontology.org/ontology/go-basic.obo\n")
        print("\t\twget ftp://ftp.ncbi.nih.gov/gene/DATA/gene2go.gz\n")
        obodag = GODag(os.path.join(script_path, "diana/toolbox/go-basic.obo"))
        geneid2gos_human = read_ncbi_gene2go(os.path.join(script_path, "diana/toolbox/gene2go"), taxids=[9606])
    ...
    data_dir = os.path.join(options.workspace, "data")
    try:
        os.stat(data_dir)
    except:
        os.mkdir(data_dir)
    ...
    random_networks_dir = os.path.join(data_dir, "random_networks")
    try:
        os.stat(random_networks_dir)
    except:
        os.mkdir(random_networks_dir)

    # Create a directory for the results
    results_dir = os.path.join(options.workspace, "results")
    try:
        os.stat(results_dir)
    except:
    ...
    for (drug, seeds) in list_seeds_drugs:
        ...
        if profile_creation_mode == True: # Skip the seed controlling for drug 2 if profile creation mode activated
            if drug == options.drug2:
                continue

        # Create a directory for each drug
        try:
            os.stat(os.path.join(data_dir, drug))
            print("  DIANA INFO:\tIt already exists a directory for %s.\n" %(drug))
        except:
            print("  DIANA INFO:\tCreating directory for %s.\n" %(drug))
            os.mkdir(os.path.join(data_dir, drug))
        ...
        if not fileExist(seeds):
            ...
            if drug != 'drug1' and drug != 'drug2' and drug != None:
                ...
                seeds = dbs.search_seeds_in_file(name, toolbox_dir, provided_seeds = None)
        ...
        seeds_dir = os.path.join(options.workspace, "seeds")
        try:
            os.stat(seeds_dir)
        except:
            print("  DIANA INFO:\tCreating directory for seeds.\n")
        ...
        seeds_file = os.path.join(seeds_dir, drug+'.newseeds.'+seeds_type)
        f = open(seeds_file,"w")
        for seed in seeds:
            f.write("%s\n" %(seed))
        f.close()
	...
	#--------------------------------------------#
        ...
        for drug in (options.drug1, options.drug2):
            ...
            if profile_creation_mode == True: # Skip the step for drug 2 if profile creation mode activated
                if drug == options.drug2:
                    continue
            ...
            nodes_file = os.path.join(data_dir, drug+"/human_nodes_BIANA_"+drug+".txt")
            edges_file = os.path.join(data_dir, drug+"/human_edges_BIANA_"+drug+".txt")
            ...
            if not fileExist(nodes_file) and not fileExist(edges_file):
                ...
                run_generate_netscore_files(data_dir, drug, seeds, options.radius, options.taxid, options.restriction, seeds_type, translation_type, options.database)
                print("  DIANA INFO:\tNetwork finished for %s.\n" %(drug))

                # Change the name and folder of the translation of seeds
                translation = os.path.join(data_dir, drug+'/translation_'+drug+'_seeds2BIANAcodes.txt')
                command = 'mv translation_seeds_to_BIANA_codes.txt '+translation
                os.system(command)

                # Create the folder where the results will be stored
                try:
                    os.stat(os.path.join(data_dir, drug+"/guild_results"))
                except:
                    print("  DIANA INFO:\tCreating directory for GUILD results of %s.\n" %(drug))
                    os.mkdir(os.path.join(data_dir, drug+"/guild_results"))
                ...
                nodes_file = os.path.join(data_dir, drug+"/human_nodes_BIANA_"+drug+".txt")
                new_seeds_file = os.path.join(data_dir, drug+"/"+drug+"_seeds_guild.txt")
                ...
                edges_file = os.path.join(data_dir, drug+"/human_edges_BIANA_"+drug+".txt")
                sif_file = os.path.join(data_dir, drug+"/network_"+drug+"_guild.sif")
                ...
                results_dir = os.path.join(drug_dir, 'guild_results')

                guild_command = "python %s/diana/toolbox/run_guild.py %s %s %s %s %s" % (script_path, drug_dir, new_seeds_file, sif_file, results_dir, random_networks_dir)
	...
	#-------------------------------#
        ...
        for drug in (options.drug1, options.drug2):
            ...
            if not fileExist(pval_file):
                ...
                guild_command = "python %s/diana/toolbox/run_guild.py %s %s %s %s %s" % (script_path, drug_dir, new_seeds_file, options.sif, results_dir, random_networks_dir)
                os.system(guild_command)
    ...
    for drug in (options.drug1, options.drug2):
        ...
        if options.sif is None:
            results_dir = os.path.join(data_dir, drug+"/guild_results")
        else:
        ...
        output_file = results_dir+"/node_profile.txt"
        if not fileExist(output_file):
            ...
            if options.sif == None or (options.sif != None and options.trans_if_sif != None and options.type_if_sif != 'geneid'):
                ...
                translation_file = os.path.join(data_dir, drug+"/translation_BIANA_%s.txt" %(drug))
            ...
            print("  DIANA INFO:\tUsing GOATOOLS for the functional analysis. Remember to update the files go-basic.obo and gene2go.\n")
            print("\t\twget http://geneontology.org/ontology/go-basic.obo\n")
            print("\t\twget ftp://ftp.ncbi.nih.gov/gene/DATA/gene2go.gz\n")
            pvalue_file = results_dir+"/output_scores.sif.netcombo.pval"
            node_profile_geneid = results_dir+"/node_profile_geneid.nodes"
            ...
            top_node_to_vals = GU.get_values_from_pvalue_file(node_profile_geneid)
            ...
            GOA.calculate_functional_enrichment_profile(obodag, geneid2gos_human, top_nodes, all_nodes, temp_file, output_file)
            ...
            print("  DIANA INFO:\tThe top scoring procedure was already done and it will be skipped.\n")
        ...
        if not fileExist(output_file):
            ...
            # GOATOOLS
            linker_node_profile_list = [ int(x) for x in linker_node_profile_list ]
            temp_file = results_dir+"/temp_enrichment_goatools.txt"
            linker_functional_profile = results_dir+"/linker_functional_profile.txt"

            GOA.calculate_functional_enrichment_profile(obodag, geneid2gos_human, linker_node_profile_list, all_nodes, temp_file, linker_functional_profile)
        ...
        if options.extended != None:
            ...
            for line in f:
                (thresholds, type_threshold) = line.strip().split('\t')
                thresholds_list = thresholds.split(',')
                top_thresholds_list.append((thresholds_list, type_threshold))
            f.close()

            print("  DIANA INFO:\tStarting EXTENDED ANALYSIS process for %s.\n" %(drug.upper()))

            import diana.toolbox.extended as ext

            # Run the Extended Analysis for the NODE PROFILE

            pvalue_file = results_dir+"/output_scores.sif.netcombo.pval"

            # If comparison mode activated, the extended analysis is skipped and we only store the profiles
            if comparison_mode == True:
                list_files = []
                for thresholds_list, type_threshold in top_thresholds_list:
                    for top in thresholds_list:
                        node_profile_file = results_dir + "/node_profile_"+str(top)+"_"+type_threshold+".txt"
                        list_files.append(node_profile_file)
                profiles_list_node.append(list_files)
            else:
            ...
            if comparison_mode == True:
                ...
                for thresholds_list, type_threshold in top_thresholds_list:
                    for top in thresholds_list:
                        functional_profile_file = results_dir + "/functional_profile_top_"+str(top)+"_"+type_threshold+".txt"
                        list_files.append(functional_profile_file)
                profiles_list_functional.append(list_files)
            else:
                #profiles_list_functional.append(ext.run_extended_analysis_functional(results_dir, top_thresholds_list))
                profiles_list_functional.append(ext.run_extended_analysis_functional_GOA(results_dir, top_thresholds_list, obodag, geneid2gos_human))
            ...
            if comparison_mode == True:
                ...
                for thresholds_list, type_threshold in top_thresholds_list:
                    for top in thresholds_list:
                        edge_profile_file = results_dir + "/edge_profile_top_"+str(top)+"_"+type_threshold+".txt"
                        list_files.append(edge_profile_file)
                profiles_list_edge.append(list_files)
            else:
    ...
    if comparison_mode == True or normal_mode == True:
        ...
        pfams1 = dbs.search_pfams_for_seeds(seeds1, options.database, toolbox_dir)
        pfams2 = dbs.search_pfams_for_seeds(seeds2, options.database, toolbox_dir)
    ...
    for (drug, seeds) in list_seeds_drugs:
        ...
        seed_functional_profile = os.path.join(data_dir, drug+"/seed_functional_profile.txt")
        ...
        if not fileExist(seed_functional_profile):
            ...
            # GOATOOLS
            temp_file = results_dir+"/temp_enrichment_goatools.txt"
            seeds_func_analysis = [ int(x) for x in seeds_func_analysis ]
            GOA.calculate_functional_enrichment_profile(obodag, geneid2gos_human, seeds_func_analysis, all_nodes, temp_file, seed_functional_profile)
    ...
    if comparison_mode == True or normal_mode == True:
        ...
        enrichment_drug1 = os.path.join(data_dir, options.drug1+"/seed_functional_profile.txt")
        enrichment_drug2 = os.path.join(data_dir, options.drug2+"/seed_functional_profile.txt")
    ...
    # Finish the analysis if the profile creation mode is activated
    if profile_creation_mode == True:
        print("\t\t----------------------------------------------------\n")
        print("\t\tProfile creation finished. Thank you for using DIANA\n")
        print("\t\t----------------------------------------------------\n")
        sys.exit(0)
    ...
    if options.sif is None:
        results_drug1 = os.path.join(data_dir, options.drug1+"/guild_results")
        results_drug2 = os.path.join(data_dir, options.drug2+"/guild_results")
    else:
        results_drug1 = os.path.join(data_dir, options.drug1+"/guild_results_using_sif")
        results_drug2 = os.path.join(data_dir, options.drug2+"/guild_results_using_sif")
    ...
    edges_dict_drug1, edges_dict_drug2 = comp.parse_edge_profiles_2_drugs(network_scored_drug1, network_scored_drug2)
    ...
    # Comparison of the Functional profile
    print("\n  DIANA INFO:\tStarting comparison between functional enrichment profiles\n")

    enrichment_drug1 = results_drug1+"/enrichment_goatools.txt"
    enrichment_drug2 = results_drug2+"/enrichment_goatools.txt"
    ...
    #summary_linker_node = comp.calculate_comparison_top_scoring(linkers_to_vals_drug1, node_to_vals_drug1, linkers_to_vals_drug2, node_to_vals_drug2, type_of_profile = 'node')
    ...
    #summary_linker_edge = comp.calculate_comparison_top_scoring(link_edges_dict_drug1, edges_dict_drug1, link_edges_dict_drug2, edges_dict_drug2, type_of_profile = 'edge')
    ...
    smiles1 = dbs.get_smiles_from_dcdbid(options.drug1, toolbox_dir)
    smiles2 = dbs.get_smiles_from_dcdbid(options.drug2, toolbox_dir)

    if smiles1 != None and smiles2 != None:
        struc_similarity = get_smiles_similarity(smiles1, smiles2, fp_type = "sub", metric = "tanimoto")
        if struc_similarity == None:
    ...
    results_folder = os.path.join(options.workspace, 'results/results_'+options.drug1+'_'+options.drug2)
    results_folder2 = os.path.join(options.workspace, 'results/results_'+options.drug2+'_'+options.drug2)
    try:
        os.stat(os.path.join(results_folder))
        print("  DIANA INFO:\tIt already exists a directory for the results of %s and %s.\n" %(options.drug1, options.drug2))
    except:
        try:
            os.stat(os.path.join(results_folder2))
            print("  DIANA INFO:\tIt already exists a directory for the results of %s and %s.\n" %(options.drug1, options.drug2))
            results_folder = results_folder2
        except:
            print("  DIANA INFO:\tCreating directory for %s and %s results.\n" %(options.drug1, options.drug2))
            os.mkdir(os.path.join(results_folder))
    ...
    if options.extended != None:
        extended_dataframe = ext.extended_analysis_comparison(profiles_list_node, profiles_list_edge, profiles_list_functional, pvalue_files, scored_networks, top_thresholds_list, output_file_general)
    ...
    functional_profile1 = results_drug1+'/enrichment_goatools.txt'
    functional_profile2 = results_drug2+'/enrichment_goatools.txt'
    ...
    output_file_table = results_folder+'/table_results_%s_%s.txt' %(options.drug1, options.drug2)
    fot = open(output_file_table, 'w')

    fot.write('# Tops: ')
    for top in extended_dataframe['node']:
        fot.write('{}\t'.format(top[0]))
    fot.write('\n')

    for top in extended_dataframe['node']:
        fot.write('{}\t{}\t'.format(top[1], top[3]))
    fot.write('{}\t{}\t'.format(summary["node"][1], summary["node"][0]))
    for top in extended_dataframe['edge']:
        fot.write('{}\t{}\t'.format(top[1], top[3]))
    fot.write('{}\t{}\t'.format(summary["edges"][1], summary["edges"][0]))
    for top in extended_dataframe['functional']:
        fot.write('{}\t{}\t'.format(top[1], top[3]))
    fot.write('{}\t{}\t'.format(summary['linker_node'][1], summary['linker_node'][0]))
    fot.write('{}\t{}\t'.format(summary['linker_edge'][1], summary['linker_edge'][0]))
    fot.write('{}\t{}\t'.format(summary['linker_functional'][1], summary['linker_functional'][0]))
    fot.write('{}\t{}\t{}\t'.format(summary_seed_node[1], summary_seed_node[0], summary_seed_node[3]))
    fot.write('{}\t{}\t{}\t'.format(summary_seed_pfam[1], summary_seed_pfam[0], summary_seed_pfam[3]))
    fot.write('{}\t{}\t'.format(summary_seed_functional[1], summary_seed_functional[0]))
    fot.write('{}\n'.format(struc_similarity))
    fot.close()
